...
for root_dir,sub_dir,files in os.walk(r"/Users/pingguo/PycharmProjects/backtrader/datas"):
    for file in files:
        if file.endswith(".xlsx") and file.__contains__("Nationality"):
            #构造绝对路径
            ...
            df=pd.read_excel(file_name,header=None).drop(0)[1:-1]
            df.columns = ['Nationality','Visa Class','Issuances']
            #print(df)
        ...
        #                 dfs = pd.concat([dfs, df])
        #         dfs = dfs[:-1]
dfs.to_csv("/Users/pingguo/PycharmProjects/backtrader/datas/visa_excel.csv")
#b = dfs[dfs[0].isin(['Beijing','Shanghai','Shenyang','Chengdu','Guangzhou'])]
# b = dfs[dfs['Nationality'].isin(['China-mainland','China - mainland'])]
